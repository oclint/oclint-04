...
bool RedundantIfStatementRule::isCIntegerViolated(
...
bool RedundantIfStatementRule::isCXXBoolViolated(
  Expr *thenExpr, Expr *elseExpr) {
  if (thenExpr && elseExpr) {
    ...
    return thenCXXBool && elseCXXBool 
      && thenCXXBool->getValue() != elseCXXBool->getValue();
  }
  return false;
}

bool RedundantIfStatementRule::isObjCBOOLViolated(
  Expr *thenExpr, Expr *elseExpr) {
  if (thenExpr && elseExpr) {
    ...
    return thenObjCBOOL && elseObjCBOOL 
      && isCIntegerViolated(
        dyn_cast<IntegerLiteral>(thenObjCBOOL->getSubExpr()), 
...
bool RedundantIfStatementRule::isNotEquals(Expr *firstExpr, Expr *secondExpr) {
  return (isCIntegerViolated(firstExpr, secondExpr) 
    || isCXXBoolViolated(firstExpr, secondExpr) 
    || isObjCBOOLViolated(firstExpr, secondExpr));
}

bool RedundantIfStatementRule::doesReturnStatementsViolateRule(
  ReturnStmt *first, ReturnStmt *second) {
  return first && second 
    && isNotEquals(first->getRetValue(), second->getRetValue());
}

bool RedundantIfStatementRule::doesBinaryOperatorsViolateRule(
  BinaryOperator *first, BinaryOperator *second) {
  return first && second && isNotEquals(first->getRHS(), second->getRHS());
}

void RedundantIfStatementRule::apply(
  ...
  if (stmt) {
    ...
    if (ifStmt) {
      ...
      BinaryOperator *thenBinaryOperator = 
        extractStmt<BinaryOperator>(ifStmt->getThen());
      BinaryOperator *elseBinaryOperator = 
        extractStmt<BinaryOperator>(ifStmt->getElse());
      if (doesReturnStatementsViolateRule(thenReturnStmt, elseReturnStmt) 
          || doesBinaryOperatorsViolateRule(
            thenBinaryOperator, elseBinaryOperator)) {
